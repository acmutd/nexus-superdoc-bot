...
class SemanticChunker(BaseDocumentTransformer):
    ...
    def split_text(
        ...
        if len(single_sentences_list) == 1:
            sentence = single_sentences_list[0]
            embedding = self.embeddings.embed_documents([sentence])[0]
            return[{
                "text": sentence,
                "mean_embedding": embedding,
                "relevant_sentence": single_sentences_list[0],
            }]
        ...
        for index in indices_above_thresh:
            ...
            chunk_dict["text"] = " ".join([d["sentence"] for d in group])
            
            #pretty simple, go through the group and mean_pool all of the emebeddings
            #to get chunk vector
            embeddings_array = np.array([d["combined_sentence_embedding"] for d in group])
            chunk_dict["mean_embedding"] = embeddings_array.mean(axis=0)
            chunk_dict["relevant_sentence"] = self.find_relevant_sentence(chunk_embedding=chunk_dict["mean_embedding"],chunk=group)
            # If specified, merge together small chunks.
            if (
                self.min_chunk_size is not None
                and len(chunk_dict["text"]) < self.min_chunk_size
            ):
                continue
        ...
        if start_index < len(sentences):
            chunk_dict = {}
            chunk_dict["text"] = " ".join([d["sentence"] for d in sentences[start_index:]])
            embeddings_array = np.array([d["combined_sentence_embedding"] for d in sentences[start_index:]])  # Fix: use current sentences
            chunk_dict["mean_embedding"] = embeddings_array.mean(axis=0)
            chunk_dict["relevant_sentence"] = self.find_relevant_sentence(
                chunk_embedding=chunk_dict["mean_embedding"],
    ...
    def create_documents(
        ...
        for i, text in enumerate(texts):
            ...
            for chunk in self.split_text(text):
                ...
                embedding = chunk.get("mean_embedding")
                metadata["chunk_embedding"] = embedding
                metadata["relevant_sentence"] = chunk["relevant_sentence"]
                if self._add_start_index:
                    metadata["start_index"] = start_index
                new_doc = Document(page_content=chunk["text"], metadata=metadata)
                documents.append(new_doc)
                start_index += len(chunk["text"])
        return documents
    ...
    def find_relevant_sentence(self, chunk_embedding, chunk: List[dict]) -> str:
        ...
        for sentence in chunk: 
            ...
            if current_similarity > max_similarity: 
                ...
                most_relevant_sent = sentence["sentence"]
        return most_relevant_sent
    
if __name__ == "__main__": 
    with open("info copy.txt") as f:
        text = f.read()
    ...
    print("FIRST DOC:"+str(documents[0]))
    print("SECOND DOC:"+str(documents[1])) 
